...
class WholeTestCase(unittest.TestCase):
    ...
    def test_version(self):
        result = subprocess.run(['python', 'rss_reader.py', '--version'], stdout=subprocess.PIPE)
        output = result.stdout.decode()
        teststr = "rss_reader.py "
        self.assertIn(teststr, str(output))

    def test_yahoo_rss_full(self):
        result = subprocess.run(['python', 'rss_reader.py', 'https://www.yahoo.com/news/rss'], stdout=subprocess.PIPE)
        output = result.stdout
        teststr = "Title: "
        self.assertIn(teststr, str(output))

    def test_yahoo_rss_full_json(self):
        result = subprocess.run(['python', 'rss_reader.py', 'https://www.yahoo.com/news/rss', '--json'],
